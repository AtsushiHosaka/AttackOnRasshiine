...
async function handleCompleteSession(payload: GameApiRequest): Promise<Response> {
  ...
  const status = evaluation.ok ? "pending" : "ai_pending";
  const updated = await updateSession(session.id, {
    ...
    status,
    suspicious_flags: [],
    ai_evaluation_failure_reason: evaluation.ok ? null : normalizeFailureReason(evaluation.reason),
  });
...
async function buildSnapshot(): Promise<Record<string, unknown>> {
  const [teamsById, users, stats, sessions, evaluations, products, achievements, auditLogs, activeBattle] = await Promise.all([
    loadTeams(),
    rest<DbUser[]>(`users?${qs({ select: userSelect(), order: "created_at.asc" })}`, { method: "GET" }),
  ...
  return {
    Users: await Promise.all(users.map((user) => userDto(user, teamsById))),
...
async function loadLatestBattleState(): Promise<BattleState | null> {
  ...
  const participantStates = await Promise.all(participants.map(async (participant) => ({
    ...participant,
...
async function buildBattleParticipants(battleId: string): Promise<Record<string, unknown>[]> {
  const members = await rest<DbUser[]>(
...
function battleDto(state: BattleState): Record<string, unknown> {
  return {
    ...
    Result: state.battle.result ?? "",
    TotalDamage: state.battle.total_damage ?? 0,
    ...
    Actions: state.actions.map(battleActionResultDto),
  };
}
